...
#화면 타이틀
pygame.display.set_caption('han game - pang') #게임 이름
...
image_path = os.path.join(current_path, "images")#이미지 폴더의 위치를 반환
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT:
                ...
                weapon_x_pos = character_x_pos + (character_width/2) - (weapon_width/2) #캐릭터 중간 위치에서 무기 발사 위치를 지정
    ...
    weapons = [[w[0], w[2] - weapon_speed] for w in weapons]#무기 위치를 위로 올림
    ...
    for ball_idx, ball_val in enumerate(balls) :
        ...
        else: #그외에는 속도를 줄여나가 0까지 된다음(멈춤) 다시 떨어지게 속도를 증가
            ball_val["to_y"] += 0.5
        ...
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx <3 :
                    ...
                    #왼쪽으로 튕겨나가는 작은공
                    balls.append({
                            "pos_x" : ball_pos_x + (ball_width/2) - (small_ball_width/2), #좌표
